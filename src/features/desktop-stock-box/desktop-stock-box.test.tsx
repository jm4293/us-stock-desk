...
describe("DesktopStockBox", () => {
  ...
  it("종목 심볼을 렌더링한다", () => {
    render(<DesktopStockBox {...defaultProps} />);
  ...
  it("닫기 버튼 클릭 시 onClose가 id와 함께 호출된다", async () => {
    const onClose = vi.fn();
    render(<DesktopStockBox {...defaultProps} onClose={onClose} />);
  ...
  it("클릭 시 onFocus가 호출된다", async () => {
    const onFocus = vi.fn();
    render(<DesktopStockBox {...defaultProps} onFocus={onFocus} />);
