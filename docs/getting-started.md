...
## 🎯 빠른 시작 (Architect Agent 실행)

다음 명령어로 Claude에게 요청하세요:

```
.claude/AGENT_ARCHITECT.md를 읽고 프로젝트 초기 설정을 완료해주세요.

다음 작업을 순서대로 진행해주세요:
1. tsconfig.json, vite.config.ts 등 설정 파일 생성
2. index.html, src/main.tsx, src/App.tsx 생성
3. src/ 폴더 구조 전체 생성
4. 기본 타입 정의 (src/types/)
5. 상수 정의 (src/constants/)
6. .env.example 생성

완료되면 npm run dev로 실행 가능한 상태로 만들어주세요.
```
...
**다음 단계**: Architect Agent를 실행하여 프로젝트 기본 구조를 만드세요! 🚀
