# 데이터 소스 전략

## 📋 개요

US Stock Desk는 두 가지 외부 데이터 소스를 사용합니다:

1. **Finnhub** - 실시간 주식 가격 (서버 측 프록시 처리)
2. **Yahoo Finance** - 차트 OHLCV 데이터 (무료, API 키 불필요)

**모든 사용자는 별도의 API 키 발급 없이 실시간 데이터를 사용할 수 있습니다.**

---

## 📡 데이터 소스별 역할

### 1. Finnhub - 실시간 주식 가격 + 차트 데이터

- **용도**: 현재가, 등락률, 거래량 등 실시간 Quote 데이터 및 차트 OHLCV 데이터
- **엔드포인트**: `/api/stock-proxy` (Vercel Serverless Function)
- **API 키 관리**: 서버 환경변수(`FINNHUB_API_KEY`)로 관리, 클라이언트에 노출 없음
- **사용자 조작 필요 없음**: 배포된 서비스에서 자동으로 처리됨
- **데이터 방식**: WebSocket (실시간) + REST API Polling (Fallback) + Candle API (차트)

```
사용자 → /api/stock-proxy → Finnhub API (서버에서 키 사용)
사용자 → Finnhub WebSocket (실시간 가격)
```

> **참고**: 초기 계획의 Yahoo Finance 차트 데이터는 Finnhub Candle API로 통합되었습니다.

---

## 🌐 사용자 경험

모든 사용자는 동일한 조건으로 서비스를 이용합니다:

- API 키 발급 불필요
- 회원가입 불필요
- 실시간/제한 모드 구분 없음
- 모든 기능 즉시 사용 가능

---

## 🔒 보안 고려사항

### API 키 관리 (서버 측)

- Finnhub API 키는 Vercel 환경변수로만 관리
- 클라이언트 번들에 API 키 포함되지 않음
- HTTPS 통신만 허용

### 개발 환경

개발자가 로컬에서 실행할 경우 `.env.local` 파일에 키를 설정해야 합니다:

```env
# .env.local (개발자 전용, 일반 사용자는 불필요)
FINNHUB_API_KEY=your_api_key_here
```

Vercel 배포 환경에서는 Vercel 대시보드에서 환경변수를 관리합니다.

---

## 📊 데이터 업데이트 전략

### 실시간 가격 (Finnhub)

```
WebSocket (1차)
  → wss://ws.finnhub.io?token=...
  → 실시간 가격 push

REST API Polling Fallback (설정에 따라 5초 / 10초 / 30초 간격)
  → /api/stock-proxy?symbol=AAPL&type=quote
  → 현재가, 등락률, 거래량 갱신
```

### 차트 데이터 (Finnhub Candle API)

```
REST API Polling
  → /api/stock-proxy?symbol=AAPL&type=candle&resolution=D&from=...&to=...
  → 기간별 OHLCV 데이터 (1일, 1주, 1개월, 3개월, 6개월, 1년)
```

---

## 🚫 제거된 기능

기존 계획에 있던 아래 기능들은 구현하지 않습니다:

- "제한 모드" / "실시간 모드" 구분
- 사용자의 개인 API 키 입력 및 검증
- 설정 모달의 API 키 입력 섹션
- 헤더의 API 키 상태 표시 (실시간/제한 배지)

---

**작성일**: 2026-02-16
